   ...
   public WoodenBroomEntity(EntityType<WoodenBroomEntity> entityType, Level level) {
    ...
    public Vec3 getInputVector(Vec3 movement, float speed, float angle) {
        ...
        if (length < 1.0E-7D) {
            ...
            assert player != null;
            boolean wearingHat = player.getItemBySlot(EquipmentSlot.HEAD).is(BroomsItems.SUCCUBA_HAT)||player.getItemBySlot(EquipmentSlot.HEAD).is(BroomsItems.SUCCUBA_HAT_FAKE);
            Vec3 vec3 = (length > 1.0D ? movement.normalize() : movement).scale(wearingHat ? speed * 2.25f : speed);
